...
const COLORS = ['#0f766e', '#ef4444', '#f59e0b', '#2563eb', '#8b5cf6', '#ec4899', '#06b6d4', '#84cc16'];
...
const ExpenseChart = () => {
  ...
  if (data.length === 0) return <div className="h-48 flex flex-col items-center justify-center gap-2" style={{ color: 'var(--text-muted)' }}><span className="text-3xl">Empty</span><p className="text-sm">No expense data this month</p></div>;

  const chartData = data.map((entry) => ({
    name: entry.category.replace(/_/g, ' ').replace(/\b\w/g, (character) => character.toUpperCase()),
    value: entry.total
  }));
    ...
    <div className="h-48">
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          ...
          <Tooltip formatter={(value) => [`Rs ${Number(value).toLocaleString('en-IN')}`, 'Amount']} />
